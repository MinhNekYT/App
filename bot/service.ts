...
export async function handleCommand(i: ChatInputCommandInteraction) {
  ...
  if (i.commandName === "logs") { if (!await admin(i, user)) return; const channel = i.options.getChannel("channel", true); if (!channel.isTextBased() || !("send" in channel)) return i.reply({ content: "Please choose a text-based channel.", ephemeral: true }); await db.setBotSetting(auditLogChannelSettingKey, channel.id); return i.reply({ content: `Command audit logging enabled for <#${channel.id}>.`, ephemeral: true }); }
...
export async function sendAuditLog(client: Client, input: { userId: string; userTag: string; commandName: string; options: Array<{ name: string; value?: unknown; options?: Array<{ name: string; value?: unknown }> }> }) { const channelId = await db.getBotSetting(auditLogChannelSettingKey); if (!channelId) return; try { const channel = await client.channels.fetch(channelId); if (channel?.isTextBased() && "send" in channel) await channel.send({ content: formatAuditCommand(input), allowedMentions: { users: [input.userId] } }); } catch (error) { console.warn("[FrierenCloud] Audit log delivery failed", error); } }
export function createDiscordBot() { const client = new Client({ intents: [GatewayIntentBits.Guilds] }); client.once(Events.ClientReady, async r => { await db.awardDuePartnerRewards(); console.info(`[FrierenCloud] Logged in as ${r.user.tag}`); r.user.setActivity("/help · Ubuntu VPS control"); }); client.on(Events.InteractionCreate, async x => { if (x.isChatInputCommand()) try { await sendAuditLog(client, { userId: x.user.id, userTag: x.user.tag, commandName: x.commandName, options: x.options.data }); await handleCommand(x); } catch (e) { const m = e instanceof Error ? e.message : "Unexpected command error."; if (x.deferred || x.replied) await x.editReply(`Command failed: ${m}`); else await x.reply({ content: `Command failed: ${m}`, ephemeral: true }); } }); return client; }
